...
class TestSecurityHeaders:
    ...
    def test_csp_nonce_format_and_entropy(self, client: TestClient):
        ...
        # style-src must be strict (no unsafe-inline, supports nonces for inline styles)
        ...
        assert "'nonce-" in style_src, (
            "style-src must contain nonce for inline <style> blocks (admin.html team/HTMX styles)"
        )
        assert "'unsafe-inline'" not in style_src, (
            "style-src must NOT contain 'unsafe-inline' (pentesting requirement)"
        )
